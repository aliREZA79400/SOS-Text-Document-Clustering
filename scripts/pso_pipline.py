...
def pso_pipeline_20news(categories=None,K:int=None,NUM_SAMPLES:int=None,       
    ...
    model = PSO.OriginalPSO(epoch=epoch,pop_size=pop_size,c1=0.5,c2= 0.3,w=0.9)
...
def pso_pipeline_bbc_sport(K:int=None,NUM_SAMPLES:int=None,       
                   max_feature:int=None,epoch:int=None,pop_size:int=None):
    ...
    sampled_data = tfidf_transformer.fit_transform(sampled_data)
    svd = TruncatedSVD(n_components=max_feature)
    X_new = svd.fit_transform(sampled_data.toarray())
